    ...
    public function havePermission($permCodeName, $pk) {
        try {
            ...
            $sql = <<<EOD
                ...
                where ((user_id = ?) and (object_pk = ?) and codename = ?) 
                        or (? in (select id from people_profile where  is_superuser))
                        or (base_resourcebase.owner_id = ?)
            EOD;
            $params = [$userId, (string)$pk, (string)$pk, $permCodeName,$userId, $userId, 
                        $userId, (string)$pk, $permCodeName, $userId, $userId];
